...
def emily_datefind_html(article):  # rename to
    global newscuesdf  # I'm confused regarding this, but OK
    try: tmp = newscuesdf
    except:
        htmldatemapfile = 'datemap.csv'
        try: newscuesdf = pd.read_csv(pathtohtmldatemap='datemap.csv',index_col=0).T
        except:
            print("   DEATH ERROR:",htmldatemapfile,"not found.")
            return ""
    if newssource in newscuesdf.columns:
        dateraw = find_html_in_between(article.html,newscuesdf[newssource][0])
...
# Returns a dictionary in the form {'datePublished': '2020-06-29T18:51:27-04:00', 'dateModified': '2020-06-29T19:52:56-04:00'}
def emily_datefind_json(article):  # rename to _json
    retval = re.findall(r"[\"\']date\w+[\"\']:\s*[\"\'][\w\-:\.\+]+[\"\']", article.html)
    ...
    retval = retval.sort_values(1).drop_duplicates(0).set_index(0)  # sort by dates in old-new order and keep oldest dates
...
def get_dates(article):
    try:
        ...
        pubtime = ""
    datedict = emily_datefind_json(article)  # method name changed
    ...
    if pubtime == "":
        pubtime = emily_datefind_html(article)
